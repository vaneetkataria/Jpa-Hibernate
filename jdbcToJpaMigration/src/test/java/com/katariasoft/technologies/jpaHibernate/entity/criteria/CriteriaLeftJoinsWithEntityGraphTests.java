...
@SpringBootTest
public class CriteriaLeftJoinsWithEntityGraphTests {
	...
	public void fetchFullDataWithJPQLQuery() {
		...
		Join<Student, Vehicle> stuVehcileJoin = insStuJoin.join(Student_.vehicles, JoinType.LEFT);
		Join<Vehicle, Document> vehicleDocumentJoin = stuVehcileJoin.join(Vehicle_.documents, JoinType.LEFT);
	...
	public void fetchFullDataWithJPQLQueryWithPartialEntityGraph() {
		...
		Root<Instructor> root = cq.from(Instructor.class);
		root.join(Instructor_.idProof, JoinType.LEFT);
		root.join(Instructor_.vehicles, JoinType.LEFT).join(Vehicle_.documents, JoinType.LEFT);
		root.join(Instructor_.students, JoinType.LEFT).join(Student_.vehicles, JoinType.LEFT);
		DataPrinters.listDataPrinter.accept(queryExecutor.fetchListForCriteriaQuery(
				cq.select(root).where(cb.greaterThan(root.get(Instructor_.id), 2)),
