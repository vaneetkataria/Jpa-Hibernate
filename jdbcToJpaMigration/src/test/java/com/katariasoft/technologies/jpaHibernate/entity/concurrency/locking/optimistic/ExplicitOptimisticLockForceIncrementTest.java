...
@SpringBootTest
public class ExplicitOptimisticLockForceIncrementTest {
	...
	public void simpleOptimisticLockTest() {
		doInTransansaction(em -> {
			Instructor instructor = em.find(Instructor.class, 16, LockModeType.OPTIMISTIC_FORCE_INCREMENT);
			...
			executeAsync(() -> {
				doInTransansaction(em_ -> {
					Instructor instructor_ = em_.find(Instructor.class, 16);
